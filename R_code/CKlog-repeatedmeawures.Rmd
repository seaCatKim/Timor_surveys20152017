...
cklogger <- read.csv("data/allcklogger_temps.csv") #from stats-CKTLTemp-allsitesdepth.rmd
...
head(cklogger)
cklogger[which(is.na(cklogger$Temp)),]
cklogger <- cklogger[-which(is.na(cklogger$Temp)),]

filter(cklogger, m.y == '6/2017' & Site == 'Urban-E' & Depth == '10m')
```
...
log_month %>%
  group_by(month, year) %>%
  levene_test(aveTemp ~ Site)

log_month %>%
  group_by(month, year) %>%
  levene_test(log10(aveTemp) ~ Site)
```
...
```{r linear model}
LM <- lm(aveTemp ~ Site * Depth , data = log_month)
summary(LM)
plot(LM)
```
...
model.lme.acor = lme(aveTemp ~ Site , 
                random = ~1 |log_id,
                correlation = corAR1(form = ~ 1|log_id, value = 0.5122),
                data = log_month,
                method = "REML")
...
Multivariate approach with lm
```{r log data wide with month_id as cols}
log_month_w <- log_month %>% 
   pivot_wider(id_cols = Site:log_id, names_from = month_id, values_from = aveTemp)

log_month_w

month_w <- log_month_w[,-c(1:3)]
str(month_w)

mod <- lm(month_w ~ 1 , data = log_month_w)
av.mod <- Anova(mod, idata = log_month_w, idesign = ~Site)
av.mod
summary(av.mod)
```
