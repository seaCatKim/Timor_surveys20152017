...
library(tidyverse)
log_crw_month <- read_csv('data/combined_log_crw_monthttemp.csv') 
...
source('functions/get_best_arima.R')
```
...
grid.arrange(plot(ARMA.31, type=c("p","smooth")),
             plot(ARMA.31, sqrt(abs(resid(.)))~fitted(.),
                                 type=c("p","smooth"),ylab=expression(sqrt(abs(resid)))),
                                       ## "sqrt(abs(resid(x)))"),
         plot(ARMA.32, resid(.,type="pearson")~month_id,
                  type=c("p","smooth")),
         qqnorm(ARMA.32,abline=c(0,1)))
```
...
ggsave('figures/Fig7_anova_season-method_plot.jpg', width = 7, height = 4)
```
