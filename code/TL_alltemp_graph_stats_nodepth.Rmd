...
# temp$m.y = factor(temp$m.y,levels(temp$m.y)[c(3,24,22,20,6,10,8,16,1,18,14,12,4,25,23,21,7,11,9,17,2,19,15,13,5)])
```
...
Using the SE values from calculating the Monthly averages
...
ggsave(here::here('figures/line_TLtemp.pdf'), width = 7, height = 4)
```
...
temp.avelog <- temp.same %>% 
   filter(Method != "sat") %>% 
   group_by(Site, m.y) %>% 
   summarise(siteave = mean(Temp), SD = sd(Temp), SE = SD/sqrt(n())) #, N = n())
...
ggsave(here("figures/line_TLtemp.pdf"), width = 7, height = 4)
```
