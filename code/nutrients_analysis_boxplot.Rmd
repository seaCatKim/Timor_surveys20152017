...
```{r tidy up the data}
# data to wide
nu_w <- reshape(nu_dat, timevar = "Nutrient..uM.", idvar = c("Sample.ID", "Pop", "Site2", "Site",
...
nu_ave <- nu_dat2 %>% 
   ...
   summarize(ave = mean(Concentration),
...
b<- ggplot(nu_dat2, aes(x = Site2, y = Concentration, fill = Depth)) + 
   geom_boxplot() + 
   geom_jitter(shape = 1, color="black", size=1, alpha=0.9) +
   facet_grid(Nutrient..uM.~., scales = "free_y") +
   labs(title = "Nutrient Concentrations", x = "Site", y = "Concentration [uM]")
b+ theme_classic() +
