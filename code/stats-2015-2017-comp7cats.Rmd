...
mean(compd.t$Healthy) # overall percent healthy coral
...
library(nlme)
h.lme <- lme((Healthy) ~ Year * Site2 *Depth, random = ~1|num_id, data = compd.t,
...
```{r compromised states averages}
comp.ave <- compd.t %>% 
   group_by(Site2, Year) %>%
   summarise_all(mean)

bl.ave <- summarySE(compd.t, "BL", c("Year", "Site2"))
ws.ave <- summarySE(compd.t, "WS", c("Year", "Site2"))
```
...
compave <- compd.t %>% 
   ...
   group_by(Site2, Depth, Transect, Year) %>% 
...
bl.se <- summarySE(compd.t, "BL", c("Year", "Site2", 'Depth'))
bl.se
...
leveneTest(sqrt(CCA)~Year*Site*Depth, compd.t) # not significant
```
```{r cca summary}
...
cca.lme <- lme((CCA) ~ Year * Site*Depth, random = ~1|num_id, data = compd.t,
             na.action = na.exclude)
cca.lme
summary(cca.lme)
...
leveneTest((TL)~Year*Site*Depth, compd.t) # not significant
...
tl.lme <- lme((TL) ~ Year * Site*Depth, random = ~1|num_id, 
              data = compd.t,
             na.action = na.exclude)
tl.lme
summary(tl.lme)
